...
public class BbsController {
	...
	public void list(@ModelAttribute PageCriteria pageCriteria, Model model, @ModelAttribute PagingMaker pagingMaker) throws Exception {
		...
		pagingMaker.setTotalData(bsvc.countData(pageCriteria));
	...
	public void read(@RequestParam("bid") int bid, @ModelAttribute PageCriteria pageCriteria, Model model) throws Exception{
		model.addAttribute(bsvc.read(bid)); //addAttribute의 키값을 지정하지 않았을 경우, key는 자동으로 클래스 이름을 소문자로 시작해서 저장됨
	}
	
	@RequestMapping(value="/modify")
	public void modifyGet(@RequestParam("bid") int bid, @ModelAttribute PageCriteria pageCriteria, Model model) throws Exception{
		model.addAttribute(bsvc.read(bid));
	}
	
	@RequestMapping(value="/modify", method=RequestMethod.POST)
	public String modifyPost(BbsVO bvo, @ModelAttribute PageCriteria pageCriteria, RedirectAttributes reAttr) throws Exception{
		...
		reAttr.addAttribute("searchType", pageCriteria.getSearchType());
		reAttr.addAttribute("keyword", pageCriteria.getKeyword());
		return "redirect:/bbs/list";
	}
	...
	public String delete(@RequestParam("bid") int bid, @ModelAttribute PageCriteria pageCriteria, RedirectAttributes reAttr) throws Exception{
